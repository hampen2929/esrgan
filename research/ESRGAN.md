...
4.4 アブレーションの検討 
提案されたESRGANの各成分の効果を検討するために、ベースラインのSRGANモデルを段階的に修正し、それらの違いを比較する。全体的な視覚的な比較を図8に示す。各列はモデルを表し、その構成を上に示している。赤色の符号は、前のモデルと比較した主な改善点を示している。以下、詳細な議論を行う。
...
BNアーティファクトを探索するために、表1に示すように、いくつかの実験を行う。1. ベースラインは、16個の残余ブロック（RB）を持つSRResNet[1]と同様であり、BN層はすべてLR空間、すなわちアップサンプリング層の前にある。ベースライン設定は、我々の実験ではBNアーティファクトを導入する可能性は低い。しかし、ネットワークがより深くなったり、HR空間に余分なBN層があったりすると（つまり、アップサンプリング層の後に）、BNアーチファクトが現れやすくなる（図1の例を参照）。ベースラインの訓練データセットを空のデータセット[17]に置き換えると、BNアーチファクトが出現する（図1の例を参照）。BN層は、テスト時には訓練データセット全体の推定された平均と分散を用いながら、訓練時にはバッチの平均と分散を用いて特徴量を正規化する。そのため、学習データセット（例えば、空のデータセット）とテストデータセットの統計量が大きく異なる場合、BN層は不快なアーチファクトを導入し、一般化能力を制限する傾向がある。GANのフレームワークで学習することで、我々の実験ではBNアーティファクトの発生確率を高めることができた。ベースラインと同じネットワーク構造を採用し、L1損失をV GG + GAN + L1損失に置き換える。BNアーティファクトが出現しやすくなり、視覚的な例を図2に示す。
